/******************************************************************************
 * AulaMind Enterprise 3.0 - PIE IA
 ******************************************************************************/

document.addEventListener("DOMContentLoaded", function() {
    "use strict";

    var CONFIG = window.PIE_CONFIG || {};
    ...
    var URLS = {
        ...
        document: function(id) { return "/pie/" + id; },
        exportLinks: function(id) { return "/pie/export-links/" + id; }
    };

    var form = document.getElementById("pieForm");
    var resultSection = document.getElementById("resultSection");
    var result = document.getElementById("pieResult");
    ...
    async function generatePie() {
        ...
        try {
            ...
            showToast("Adecuación PIE generada correctamente.");
    ...
    async function loadHistory() {
        ...
        try {
            ...
            if (!response.items || response.items.length === 0) {
                historyBody.innerHTML = "<tr><td colspan=\"4\" class=\"empty-state\">No existen adecuaciones PIE.</td></tr>";
                return;
            }
    ...
    async function deleteDocument(documentId) {
        if (!confirm("¿Desea eliminar esta adecuación PIE?")) return;
        showLoading();
        try {
            await request("/pie/delete/" + documentId, { method: "POST", body: JSON.stringify({ csrf_token: CSRF_TOKEN }) });
            if (currentDocument === documentId) clearResult();
            await loadHistory();
            showToast("Adecuación PIE eliminada.");
        } catch (error) {
            showToast(error.message, "error");
        } finally {
            hideLoading();
    ...
    function exportWord() {
        if (!exportInfo.word) { showToast("Debe generar o abrir una adecuación.", "error"); return; }
        window.location.href = exportInfo.word;
    }

    function exportPdf() {
        if (!exportInfo.pdf) { showToast("Debe generar o abrir una adecuación.", "error"); return; }
        window.location.href = exportInfo.pdf;
    }

    if (form) form.addEventListener("submit", function(e) { e.preventDefault(); generatePie(); });
    
    var copyBtn = document.getElementById("copyPie");
    if (copyBtn) copyBtn.addEventListener("click", copyPie);
